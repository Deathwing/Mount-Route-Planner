...
local hideMinimapButtonToggle = CreateFrame("CheckButton", nil, optionsFrame, "InterfaceOptionsCheckButtonTemplate")
hideMinimapButtonToggle:SetPoint("TOPLEFT", ignoreLFRDifficultyToggle, "BOTTOMLEFT", 0, -12)
hideMinimapButtonToggle.Text:SetText(L["Hide Minimap Button"])

hideMinimapButtonToggle:SetScript("OnEnter", function(self)
    GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
    GameTooltip:SetText(L["Hide Minimap Button"], 1, 1, 1)
    GameTooltip:AddLine(L["Enable this to hide the Mount Route Planner button on the minimap. You can still open the panel with the /mrp command."], 1, 1, 1)
    GameTooltip:Show()
end)

hideMinimapButtonToggle:SetScript("OnLeave", function()
    GameTooltip:Hide()
end)

hideMinimapButtonToggle:SetScript("OnClick", function(self)
    MRP_Settings.hideMinimapButton = self:GetChecked()
    if MRP.Minimap then
        MRP.Minimap:Refresh()
    end
end)
...
optionsFrame:SetScript("OnShow", function()
    ...
    hideMinimapButtonToggle:SetChecked(MRP_Settings.hideMinimapButton)
end)
